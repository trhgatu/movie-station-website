...
const MediaDetail: React.FC<MediaDetailProps> = ({ media, type, isLoading, error }) => {
  ...
  useEffect(() => {
    if (media && !isMetaDataUpdated) {
      document.title = `${media.title || media.name}`;
      const metaDescription = media.overview || "No description available.";
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={media.overview || "No description available."} />
      </Head>
